...
then
  ...
  # Delete the files generated by cmake except README
  # which is required by autoreconf
  find . -type f -name \*.in | while read file
  do
    base=`basename ${file} .in`
    dir=`dirname ${file}`
    if [ -f ${dir}/${base} -a ${base} != "README" ]
    then
